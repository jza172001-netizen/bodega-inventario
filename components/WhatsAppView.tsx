...
export const WhatsAppView: React.FC<Props> = ({ movements, items, personnel, readOnly = false, onBehaviorLog, onAuditLog }) => {
    ...
    const PersonCard: React.FC<{ g: PersonGroup; highlight?: boolean }> = ({ g, highlight }) => {
        ...
        return (
            <div className={`rounded-2xl p-4 border flex items-center gap-3 transition-all ${
                highlight ? 'bg-bien-suave border-bien ring-2 ring-bien' : g.isDue ? 'bg-bien-suave border-bien' : 'bg-papel border-papel-borde'
            }`}>
                <div className="w-10 h-10 rounded-full bg-marca-suave text-marca-oscuro flex items-center justify-center font-black text-base flex-shrink-0">
                    {g.person.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-bold text-tinta">{g.person.name}</p>
                        {g.isDue && (
                            <span className="text-[10px] font-black bg-bien text-papel px-1.5 py-0.5 rounded-full animate-pulse">recordar</span>
                        )}
                    </div>
                    <p className="text-xs text-tinta-tenue truncate mt-0.5">{itemNames}</p>
                    <div className="flex items-center gap-2 flex-wrap mt-0.5">
                        {g.loans.length > 0 && (
                            <span className={`text-[10px] font-semibold ${maxDays > 14 ? 'text-alerta' : maxDays > 7 ? 'text-atencion' : 'text-tinta-tenue'}`}>
                                {maxDays}d fuera
                            </span>
                        )}
                ...
                {g.hasPhone ? (
                    <div className="flex flex-col gap-1 flex-shrink-0">
                        ...
                        <button onClick={() => { onBehaviorLog?.('BUTTON', `Pidió recordatorio mes: ${g.person.name}`); confirmarEnvio(g, 30, 'all'); }}
                            className="flex items-center gap-1 px-3 py-2 text-xs font-black bg-marca-suave hover:bg-marca-suave text-marca-oscuro rounded-xl transition-all">
                            📅 Mes
                        </button>
                    </div>
                ) : (
                    <span className="flex-shrink-0 text-[10px] text-tinta-tenue text-right max-w-[72px] leading-tight">Sin número en Personal</span>
                )}
            </div>
        );
    };
    ...
    return (
        <div className="space-y-4 max-w-2xl mx-auto pr-6">
            {readOnly && (
                <div className="flex items-center gap-2 bg-atencion-suave border border-atencion rounded-xl px-4 py-2.5 text-xs font-semibold text-atencion">
            ...
            {queue && (
                <div className="sticky top-0 z-20 bg-bien text-papel px-4 py-3 rounded-2xl flex items-center justify-between gap-3 shadow-lg mb-3">
                    <div>
                        <p className="font-black text-sm">Paso {step + 1} de {queue.batch.length} — {queue.batch[step]?.person.name}</p>
                        <p className="text-xs text-papel">WhatsApp abierto → toca Enviar → vuelve aquí</p>
                    </div>
                    <button
                        onClick={nextStep}
                        className="flex-shrink-0 px-4 py-2 bg-papel text-bien font-black text-xs rounded-xl hover:bg-bien-suave transition-colors"
                    >
            ...
            <div className="flex items-start justify-between gap-3">
                <div>
                    <h1 className="text-xl font-black text-tinta">📱 WhatsApp Recordatorios</h1>
            ...
            {hasPhoneUsers && activeLoans.length > 0 && (
                <div>
                    <p className="text-[10px] font-black text-tinta-tenue uppercase tracking-widest mb-2">
                                ...
                                <button
                                    ...
                                    className={`flex-shrink-0 px-3 py-2 text-xs font-black rounded-xl transition-all ${
                                        noData
                                            ...
                                            : done
                                                ? `${btn.active} text-papel opacity-70`
                                                : `${btn.active} text-papel`
                                    }`}
            ...
            {activeLoans.length === 0 ? (
                <div className="text-center py-20 bg-papel rounded-2xl border border-papel-borde text-tinta-tenue">
                ...
                <>
                    {/* Pendientes */}
                    {dueGroups.length > 0 && (
                        <div className="bg-papel rounded-2xl border border-bien overflow-hidden shadow-sm">
                            <div className="bg-bien px-4 py-3 flex items-center justify-between gap-3">
                                <div>
                                    <p className="text-sm font-black text-papel">🔔 Necesitan recordatorio</p>
                                    <p className="text-xs text-papel">Sin contactar hace +{REMINDER_INTERVAL_DAYS} días</p>
                                </div>
                                {!queue && (
                                    <button
                    ...
                    {dueGroups.length === 0 && (
                        <div className="bg-bien-suave border border-bien rounded-2xl p-4 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-black text-bien">Al día con los recordatorios</p>
                                <p className="text-xs text-bien">Nadie lleva más de {REMINDER_INTERVAL_DAYS} días sin recibir mensaje.</p>
                            </div>
                        </div>
                    )}
                    ...
                    {onTrackGroups.length > 0 && (
                        <div>
                            <p className="text-xs font-black text-tinta-tenue uppercase tracking-widest mb-2">📋 Préstamos activos — al día</p>
                    ...
                    {noPhoneGroups.length > 0 && (
                        <div>
                            <p className="text-xs font-black text-tinta-tenue uppercase tracking-widest mb-2">⚠️ Sin número de WhatsApp</p>
                            <div className="space-y-2">
                                {noPhoneGroups.map(g => wrapCard(g, <PersonCard key={g.person.id} g={g} />))}
                            </div>
                            <p className="text-[10px] text-tinta-tenue mt-2 text-center">
                        ...
                        return (
                            <button
                                ...
                                className={`w-5 h-5 flex items-center justify-center text-[10px] font-black rounded-full transition-all ${
                                    active
                                        ? 'text-bien hover:text-papel hover:bg-bien cursor-pointer'
