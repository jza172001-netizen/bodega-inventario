...
export const FloatingChat: React.FC<FloatingChatProps> = ({
    ...
    const handleSend = async (text: string) => {
        ...
        // 1. Creation intents (single project / personnel / item)
        ...
        // 2. Bulk add intent (list of items, optional worker + project)
        ...
        // 3. Exit intent (checkout existing items)
        ...
        if (parsed.isExitIntent) {
            if (parsed.movements.length === 0 || parsed.movements.every(m => !m.matchedItem && !m.candidates.length)) {
                addBot('No pude identificar ningún material. Intenta: "saqué 5 cascos de seguridad, trabajador Pedro".');
            } else {
    ...
    const handleConfirmBulk = (msg: ChatMsg, withDispatch: boolean) => {
        ...
        // Log CHECK_IN movements (add to inventory)
        ...
        // Log CHECK_OUT movements if dispatching
        if (withDispatch && (bulk.matchedPersonnel || bulk.rawPersonnel)) {
            let worker = bulk.matchedPersonnel;
            if (!worker && bulk.rawPersonnel) {
                worker = onCreatePersonnel({ name: bulk.rawPersonnel });
            }
            let project = bulk.matchedProject;
            if (!project && bulk.rawProject) {
                project = onCreateProject({ name: bulk.rawProject, status: 'active' });
            }
