...
const Section: React.FC<{ icon: string; title: string; children: React.ReactNode; defaultOpen?: boolean }> = ({ icon, title, children, defaultOpen = false }) => {
    ...
    return (
        <div className="bg-papel rounded-2xl border border-papel-borde shadow-sm overflow-hidden">
            <button
                onClick={() => setOpen(o => !o)}
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-sm font-black text-tinta uppercase tracking-tight">{title}</h2>
                </div>
                <svg className={`w-4 h-4 text-tinta-tenue transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
                </svg>
            </button>
            {open && <div className="px-5 pb-5 space-y-3 text-sm text-tinta-suave leading-relaxed border-t border-papel-borde">{children}</div>}
        </div>
    );
};
...
const Row: React.FC<{ icon: string; label: string; desc: string }> = ({ icon, label, desc }) => (
    <div className="flex gap-3 pt-3">
        ...
        <div>
            <span className="font-bold text-tinta">{label} — </span>
            <span>{desc}</span>
        </div>
    </div>
);
...
const Tip: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <p className="bg-marca-suave border border-marca-borde rounded-xl px-3 py-2 text-xs font-semibold text-marca-oscuro mt-2">{children}</p>
);

export const HelpView: React.FC = () => (
    <div className="space-y-4 max-w-2xl mx-auto">
        <div className="text-center py-4">
            <p className="text-2xl font-black text-tinta uppercase tracking-tight">Centro de ayuda</p>
            <p className="text-sm text-tinta-tenue mt-1">Guía rápida de uso — Bodega Grupo Montecielo</p>
        </div>
            ...
            <div className="mt-3 space-y-4">
                ...
                <div className="bg-tinta rounded-xl p-3 text-papel">
                    ...
                    <div className="mt-2 space-y-1 text-xs text-tinta-tenue">
                        <p><strong className="text-tinta">Paso 1:</strong> Elige el o los tipos de ítem (manual, eléctrica, EPP, consumible — puedes marcar varios).</p>
                        <p><strong className="text-tinta">Paso 2:</strong> Elige el trabajador. Si es un <em>oficial</em>, aparece otro paso para elegir cuál de sus trabajadores se lleva la herramienta. Puedes crear un trabajador nuevo directamente desde aquí.</p>
                        <p><strong className="text-tinta">Paso 3:</strong> Asigna un proyecto (opcional). Si no existe, créalo en el momento.</p>
                        <p><strong className="text-tinta">Paso 4:</strong> Selecciona los ítems y cantidades. Si un ítem no existe, toca <em>"+ Crear nuevo"</em> para agregarlo al inventario y al despacho en el mismo paso.</p>
                        <p><strong className="text-tinta">Confirmar:</strong> Revisa el resumen, ajusta la fecha si es retroactiva, y confirma. Se registra todo de un golpe.</p>
                    </div>
                ...
                {/* Rápido */}
                <div className="bg-marca rounded-xl p-3 text-tinta">
                    <p className="font-black text-sm">⚡ Rápido — un ítem al instante</p>
                    <p className="text-xs text-tinta mt-1">Para cuando solo necesitas asignar un ítem rápidamente, sin wizard. Todo en una sola pantalla:</p>
                ...
                <div className="bg-bien rounded-xl p-3 text-papel">
                    ...
                    <div className="mt-2 space-y-1 text-xs text-tinta">
                        <p><strong className="text-tinta">Paso 1:</strong> Elige el o los tipos de ítem a agregar.</p>
                        <p><strong className="text-tinta">Paso 2:</strong> Para cada tipo, usa <em>"+ Crear nuevo"</em> para agregar un género. Para herramientas, agrega las especies (color + marca). El formulario queda abierto para que puedas crear el siguiente género sin volver atrás.</p>
        ...
        <Section icon="❓" title="Preguntas frecuentes">
            <div className="pt-2 space-y-4">
                <div>
                    <p className="font-bold text-tinta">¿Cómo marco una herramienta como devuelta?</p>
                    <p>En Kardex → Préstamos → botón "✓ Devuelta" junto al ítem. Se registra la condición en que regresó (buena, dañada, etc.).</p>
                </div>
                <div>
                    <p className="font-bold text-tinta">¿Qué diferencia hay entre préstamo y salida?</p>
                ...
                <div>
                    <p className="font-bold text-tinta">¿Cómo asigno una herramienta a un trabajador de una cuadrilla?</p>
                ...
                <div>
                    <p className="font-bold text-tinta">¿Puedo registrar una salida de una fecha anterior?</p>
                    <p>Sí. En el paso de confirmación del Despacho hay un campo de fecha — cámbiala a la fecha real del movimiento.</p>
                </div>
                <div>
                    <p className="font-bold text-tinta">¿Qué pasa si elimino algo por error?</p>
                ...
                <div>
                    <p className="font-bold text-tinta">¿Los datos se guardan automáticamente?</p>
