...
const SESSION_KEY = 'bpro_session';

const App: React.FC = () => {
    // ── Autenticación persistida ──────────────────────────────────────────
    const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
        try { return localStorage.getItem(`${SESSION_KEY}_auth`) === 'true'; } catch { return false; }
    });
    const [userRole, setUserRole] = useState<UserRole>(() => {
        try { return (localStorage.getItem(`${SESSION_KEY}_role`) as UserRole) || UserRole.OWNER; } catch { return UserRole.OWNER; }
    });

    // ── Vista persistida ──────────────────────────────────────────────────
    const [currentView, setCurrentView] = useState<View>(() => {
        try { return (localStorage.getItem(`${SESSION_KEY}_view`) as View) || 'dashboard'; } catch { return 'dashboard'; }
    });
    const [selectedInventoryType, setSelectedInventoryType] = useState<InventoryType | null>(() => {
        try { return (localStorage.getItem(`${SESSION_KEY}_invType`) as InventoryType) || null; } catch { return null; }
    });

    // ── Persistir vista al cambiar ────────────────────────────────────────
    useEffect(() => {
        if (isAuthenticated) {
            localStorage.setItem(`${SESSION_KEY}_view`, currentView);
            if (selectedInventoryType) {
                localStorage.setItem(`${SESSION_KEY}_invType`, selectedInventoryType);
            } else {
                localStorage.removeItem(`${SESSION_KEY}_invType`);
            }
        }
    }, [currentView, selectedInventoryType, isAuthenticated]);
    ...
    const handleLogin = (role: UserRole) => {
        ...
        localStorage.setItem(`${SESSION_KEY}_auth`, 'true');
        localStorage.setItem(`${SESSION_KEY}_role`, role);
    };

    const handleLogout = () => {
        setIsAuthenticated(false);
        localStorage.removeItem(`${SESSION_KEY}_auth`);
        localStorage.removeItem(`${SESSION_KEY}_role`);
    };
    ...
    const selectInventoryView = (type: InventoryType) => {
    ...
    return (
        <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
            <aside className={`bg-white border-r border-gray-200 shadow-xl transition-all duration-300 ease-in-out fixed md:relative inset-y-0 left-0 z-30 transform md:transform-none ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:${isSidebarOpen ? 'w-64' : 'w-0 overflow-hidden'} flex flex-col`}>
                ...
                <div className="flex-1 overflow-y-auto py-4">
                    <nav className="px-2 space-y-1">
                        ...
                        <NavItem icon={HandToolIcon} label="H. Manual" onClick={() => selectInventoryView(InventoryType.HAND_TOOL)} isActive={currentView === 'inventory' && selectedInventoryType === InventoryType.HAND_TOOL} />
                        <NavItem icon={ElectricalToolIcon} label="H. Eléctrica" onClick={() => selectInventoryView(InventoryType.ELECTRICAL_TOOL)} isActive={currentView === 'inventory' && selectedInventoryType === InventoryType.ELECTRICAL_TOOL} />
                        <NavItem icon={PpeIcon} label="Seguridad" onClick={() => selectInventoryView(InventoryType.PPE)} isActive={currentView === 'inventory' && selectedInventoryType === InventoryType.PPE} />
                        <NavItem icon={SingleUseIcon} label="Consumibles" onClick={() => selectInventoryView(InventoryType.SINGLE_USE)} isActive={currentView === 'inventory' && selectedInventoryType === InventoryType.SINGLE_USE} />
            ...
            <LogMovementModal
                ...
                onLogMovement={(m, date) => {
                    const timestamp = date || new Date();
                    setMovements(p => [{ ...m, id: `mov-${Date.now()}`, timestamp }, ...p]);
